...
class TestSemanticCacheSafety:
    ...
    @pytest.mark.parametrize(
        "query",
        ["降噪耳机怎么挑", "登山杖看哪些参数", "帆布和尼龙材质区别"],
    )
    async def test_only_stable_advice_queries_are_cacheable(self, query):
        assert is_cacheable_query(query) is True

    @pytest.mark.parametrize(
        "query",
        ["300 元以内的露营灯推荐", "美国免税额度是多少", "旅行三件套"],
    )
    async def test_dynamic_catalog_and_policy_queries_not_cacheable(self, query):
        assert is_cacheable_query(query) is False
...
class TestSemanticCacheHit:
    async def test_same_query_hits(self):
        ...
        await sem.remember("b1", "降噪耳机怎么挑", "先看降噪深度和续航", has_history=False)

        hit = await sem.lookup("b1", "降噪耳机怎么挑", has_history=False)
        assert hit is not None
        assert hit.reply == "先看降噪深度和续航"
        assert hit.similarity >= 0.95

    async def test_punctuation_variant_hits(self):
        """归一化后带/不带问号的同一问句应共享缓存。"""
        cache = InMemoryCache()
        sem = SemanticCache(cache, CountingEmbedder(), threshold=0.95)
        await sem.remember("b1", "降噪耳机怎么挑", "看 ANC 和续航", has_history=False)
        assert await sem.lookup("b1", "降噪耳机怎么挑？", has_history=False) is not None

    async def test_buyers_are_isolated(self):
        """回复里可能含买家偏好与地址，不能跨买家复用。"""
        cache = InMemoryCache()
        sem = SemanticCache(cache, CountingEmbedder(), threshold=0.9)
        await sem.remember("b1", "露营灯怎么挑", "按你不要塑料的偏好给建议", has_history=False)
        assert await sem.lookup("b2", "露营灯怎么挑", has_history=False) is None

    async def test_no_hit_when_session_has_history(self):
        cache = InMemoryCache()
        sem = SemanticCache(cache, CountingEmbedder(), threshold=0.9)
        await sem.remember("b1", "露营灯怎么挑", "看防水和续航", has_history=False)
        assert await sem.lookup("b1", "露营灯怎么挑", has_history=True) is None

    async def test_error_reply_not_cached(self):
        cache = InMemoryCache()
        sem = SemanticCache(cache, CountingEmbedder(), threshold=0.9)
        await sem.remember("b1", "露营灯怎么挑", "[error] 上游超时", has_history=False)
        assert await sem.lookup("b1", "露营灯怎么挑", has_history=False) is None
...
class TestPreferenceScopedCache:
    ...
    async def test_scope_change_invalidates(self):
        ...
        sem = SemanticCache(cache, CountingEmbedder(), threshold=0.9)
        await sem.remember(
            "b1", "旅行装备材质怎么挑", "按你不要塑料的偏好给建议",
            has_history=False, scope="pref-v1",
        )

        # 偏好撤回后指纹变了，旧回复不得命中
        assert await sem.lookup(
            "b1", "旅行装备材质怎么挑", has_history=False, scope="pref-v2",
        ) is None
    ...
    async def test_same_scope_still_hits(self):
        ...
        sem = SemanticCache(cache, CountingEmbedder(), threshold=0.9)
        await sem.remember(
            "b1", "旅行装备材质怎么挑", "看自重与耐用性", has_history=False, scope="pref-v1",
        )
        hit = await sem.lookup(
            "b1", "旅行装备材质怎么挑", has_history=False, scope="pref-v1",
        )
        assert hit is not None and hit.reply == "看自重与耐用性"

    async def test_empty_scope_isolated_from_nonempty(self):
        """从无偏好到有偏好（首次记住）同样要失效。"""
        cache = InMemoryCache()
        sem = SemanticCache(cache, CountingEmbedder(), threshold=0.9)
        await sem.remember("b1", "旅行装备材质怎么挑", "无偏好时的建议", has_history=False)
        assert await sem.lookup(
            "b1", "旅行装备材质怎么挑", has_history=False, scope="pref-v1",
        ) is None
