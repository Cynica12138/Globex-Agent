...
class TestProductRepository:
    async def test_seed_and_roundtrip_product_facts(self, engine):
        repo = SqlProductRepository(engine)
        assert await repo.seed_if_empty(build_seed_products()) is True
        assert await repo.seed_if_empty(build_seed_products()) is False

        product = await repo.find_by_id("P1001")
        assert product is not None
        assert product.primary_sku().price.amount_in_minor_units == 18900
        assert product.primary_sku().stock == 50
        assert product.primary_sku().version == 1

    async def test_price_change_makes_old_snapshot_stale(self, engine):
        repo = SqlProductRepository(engine)
        await repo.seed_if_empty(build_seed_products())
        product = await repo.find_by_id("P1001")
        old_version = product.primary_sku().version

        await repo.update_sku_price(
            "P1001", "P1001-S1", Money(amount_in_minor_units=19900, currency="CNY"),
        )
        with pytest.raises(ValueError, match="SKU 信息已变化"):
            await repo.reserve_stock("P1001", "P1001-S1", 1, old_version)

    async def test_reserve_stock_is_persisted_and_versioned(self, engine):
        repo = SqlProductRepository(engine)
        await repo.seed_if_empty(build_seed_products())
        product, sku = await repo.reserve_stock("P1001", "P1001-S1", 2, expected_version=1)
        assert product.product_id == "P1001"
        assert sku.stock == 48
        assert sku.version == 2
        restored = await repo.find_by_id("P1001")
        assert restored.find_sku("P1001-S1").stock == 48
