...
- 持久化：SQLite（SQLAlchemy 2.0 async）存对话/事件/会话状态/商品事实/订单/偏好；
  商品种子只在空库首启时导入，价格、库存、上下架后续可独立变化
...
- **语义缓存**：只复用“怎么挑/看哪些参数”等稳定选购知识；商品推荐、价格、
  库存、关税法规、写操作与上下文依赖问句不入最终回复缓存，避免变价后返回旧答案；
  命中会发 `cache.hit` 事件，并按 buyer 与偏好指纹分桶
- **召回与交易事实分层**：Qdrant 只负责召回 `product_id`，价格、库存和上下架
  每次从 SQLite `products/skus` 还原；下单用带库存下限的原子 UPDATE 预占库存，
  并通过 SKU `version` 拒绝变价后的旧确认卡
- **存储可替换**：`ProductRepository` / `SessionStore` / `ConversationStore` / `OrderRepository` /
  `PreferenceStore` 五个端口，
...
- `GET  /demo/catalog/target` 查看 P1001 的当前价格/库存/状态/版本
- `POST /demo/catalog/scenarios/{scenario}` 触发 `price-rise` / `out-of-stock` / `off-shelf` / `reset`
- `GET  /health` 健康检查

### Demo 时效数据演示

前端顶部可直接触发 P1001 的涨价、售罄、下架与重置。推荐演示步骤：

1. 先搜索 P1001，记住商品卡中的价格和 `v1`。
2. 点击“价格 +10”或“S1 售罄”，模拟上游平台同步。
3. 旧版本下单会被拒绝，重新搜索才能获得最新事实并再次确认。

这些数据会明确标记为 `data_mode=demo`，不伪装成真实平台实时价。品类 RAG 也标记为
`demo_reference`；法规、关税和禁限运信息需通过联网工具或官方源复核。公网部署时可设
`DEMO_SCENARIOS_ENABLED=0` 关闭变更入口。

## 验证

```bash
uv run pytest                          # 全量单测：domain / 召回 / SQL 事实库 / 订单 / 记忆 / 护栏
