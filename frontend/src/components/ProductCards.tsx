        ...
        <article key={card.product_id} className="card">
          <header>
            ...
            <span className="freshness">
              Demo 事实 · {card.source} · 更新于 {formatTime(card.source_updated_at)}
            </span>
          </header>
          <div className="price">
          ...
          <div className="skus">
            {card.skus.map((sku) => (
              <span key={sku.sku_id} className={`sku ${sku.stock === 0 ? "sold-out" : ""}`}>
                {sku.spec} · {sku.price_major} {sku.currency} · {sku.stock === 0 ? "已售罄" : `库存 ${sku.stock}`} · v{sku.version}
              </span>
            ))}
          </div>
        </article>
