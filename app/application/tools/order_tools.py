...
def build_create_order_tool(usecase: PlaceOrderUseCase, bus: TradeEventBus):
    async def create_order_tool(
        ...
        Args:
            items (`list[dict]`):
                订单行列表，每项形如 {"product_id": "P1001", "sku_id": "P1001-S1",
                "quantity": 1, "expected_version": 1}。expected_version 必须透传确认卡中的
                SKU version；期间变价、库存变化或下架时将拒绝旧快照下单。
            shipping_address (`dict`):
        ...
        try:
            if any(item.get("expected_version") is None for item in items):
                raise ValueError(
                    "create_order_tool.items[].expected_version 必填；"
                    "请重新搜索获取最新 SKU version 并让买家确认",
                )
            order_items = [
                OrderItemInput(
