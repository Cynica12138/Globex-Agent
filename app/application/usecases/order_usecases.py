...
TradeAgent 的工具层只做参数搬运，业务规则（下单前重新校验、库存预占、
状态机、金额计算）全部收敛在这里与 Order 聚合内。
"""
...
class PlaceOrderUseCase:
    ...
    async def execute(self, buyer_id: str, items: list[OrderItemInput], shipping_address: Address) -> dict:
        ...
        reserved: list[OrderItemInput] = []  # 已预占的行，失败时回补
        try:
            for item in items:
                # 确认卡与真正下单之间可能发生变价/库存变化/下架。
                # 以事实库返回的最新 Product/Sku 生成订单行，不信任之前搜索卡片的快照。
                product, sku = await self._product_repo.reserve_stock(
                    item.product_id,
                    item.sku_id,
                    item.quantity,
            ...
            for item in reversed(reserved):
                await self._product_repo.restore_stock(
                    item.product_id, item.sku_id, item.quantity,
                )
            raise
...
class CancelOrderUseCase:
    ...
    async def execute(self, order_id: str, reason: str) -> dict:
        ...
        for line in order.lines:
            await self._product_repo.restore_stock(
                line.product_id, line.sku_id, line.quantity,
            )
        await self._order_repo.save(order)
        return order.snapshot()
