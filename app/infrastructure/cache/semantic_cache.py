...
安全边界（很重要，不能放宽）：
    1. 只缓存不含时效事实的稳定选购知识（如“怎么挑”）。商品推荐、价格、
       库存、关税法规以及下单/取消一律不进最终回复缓存——
...
# 动态商品/政策事实不能用 24h 最终回复缓存，否则变价或政策变更后会继续答旧值。
_DYNAMIC_FACT_RE = re.compile(
    r"推荐|找|买|价格|多少钱|库存|现货|到手价|运费|关税|免税|法规|政策|规则|额度|热卖|流行|现在|当前",
)
_STABLE_ADVICE_RE = re.compile(r"怎么挑|如何选|看哪些|区别|优缺点|避坑|材质|参数")
...
def is_cacheable_query(query: str) -> bool:
    """仅缓存稳定选购方法，动态事实每次重走工具链。"""
    return bool(
        _STABLE_ADVICE_RE.search(query)
        and not _UNSAFE_RE.search(query)
        and not _DYNAMIC_FACT_RE.search(query)
    )
