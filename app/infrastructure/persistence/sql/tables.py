...
落库范围：对话记录 + 会话状态 + 商品/SKU 交易事实 + 订单 + 买家偏好。
向量库只负责召回 product_id，价格、库存、上下架状态每次从关系库还原，
避免把可变事实固化在向量索引中。
...
class ProductRow(Base):
    ...
    highlights_json: Mapped[list] = mapped_column(JSON)
    ships_to_json: Mapped[list] = mapped_column(JSON)
...
class SkuRow(Base):
    """SKU 快变事实：价格、库存、版本及各自的更新时间。"""

    __tablename__ = "skus"

    sku_id: Mapped[str] = mapped_column(String(32), primary_key=True)
    product_id: Mapped[str] = mapped_column(
        String(32), ForeignKey("products.product_id"), index=True,
    )
    ...
    price_updated_at: Mapped[datetime] = mapped_column(DateTime)
    stock_updated_at: Mapped[datetime] = mapped_column(DateTime)
