...
class Container:
    ...
    demo_catalog: DemoCatalogUseCase
    ...
    async def startup(self) -> None:
        ...
        if self.db_engine is not None:
            try:
                ...
                imported = await self.product_repo.seed_if_empty(build_seed_products())
                if imported:
                    logger.info("商品事实库为空，已导入固定 Demo 种子")
...
async def build_container() -> Container:
    ...
    if db_engine is not None:
        product_repo: ProductRepository = SqlProductRepository(db_engine)
    ...
    demo_catalog = DemoCatalogUseCase(product_repo, build_seed_products)
