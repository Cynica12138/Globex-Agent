...
路由：
    ...
    GET  /demo/catalog/target               查看 Demo 目标商品的当前事实
    POST /demo/catalog/scenarios/{scenario} 触发变价/售罄/下架/重置
...
def build_app() -> FastAPI:
    ...
    api = FastAPI(title="Globex 跨境电商 Agent", version="0.5.0", lifespan=lifespan)
    ...
    async def health() -> dict:
        ...
        return {
            ...
            "catalog_store": type(c.product_repo).__name__,
            "catalog_data_mode": "demo",
            "demo_scenarios": c.settings.demo_scenarios_enabled,
        }
    ...
    @api.get("/demo/catalog/target")
    async def get_demo_catalog_target() -> dict:
        c = container()
        if not c.settings.demo_scenarios_enabled:
            raise HTTPException(status_code=404, detail="Demo 场景已关闭")
        return await c.demo_catalog.get_target()

    @api.post("/demo/catalog/scenarios/{scenario}")
    async def apply_demo_catalog_scenario(scenario: str) -> dict:
        c = container()
        if not c.settings.demo_scenarios_enabled:
            raise HTTPException(status_code=404, detail="Demo 场景已关闭")
        try:
            return await c.demo_catalog.execute(scenario)
        except ValueError as err:
            raise HTTPException(status_code=400, detail=str(err)) from err
