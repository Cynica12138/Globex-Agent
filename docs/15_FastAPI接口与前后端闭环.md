...
2. 处理 `active_tasks` 任务表 + 取消语义 + WebSocket 重连。
3. 给一份 React 前端的 AGUI 消费骨架（不堆 UI，重点是事件流）。
4. 跑一次端到端 demo，把"跨 4 平台搜旅行收纳袋"的链路跑完。
...
UI 不堆样式——核心是把 `fork`、`tool_start/end`、`task_result` 三类事件**显式可见**。这正是 AGUI 事件流的真正价值：用户能看见"什么时候 Globex 自己干、什么时候叫了一群分身"。
