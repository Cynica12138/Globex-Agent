**Globex Agent 项目开发文档｜第 3 章：工程底座搭建——配置、上下文、实时监控与模型提示词体系**
...
但是！如果想让子 Agent **写自己的 thread_id**（用于 checkpoint 隔离）但**继承主 loop 的 session_dir**（保证文件落到同一个目录），就需要在 `dispatch_tool` 内部显式覆盖：  
```python  
